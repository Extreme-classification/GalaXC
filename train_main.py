...
head_net = DeepXMLBase(params["num_labels"], params["hidden_dims"], params["devices"], "dummy",
...
#introduce the novel tst points in NR scenario
if(RUN_TYPE == "NR"):
...
# mode_size = os.stat("{}/GraphXMLModel{}/model_state_dict.pt".format(DATASET, RUN_TYPE)).st_size
# print("Model size(GB): ", model_size / (1024 * 1024 * 1024))
print("==================================================================")
if(args.save_model == 1):
    model_dir = "{}/GraphXMLModel{}".format(DATASET, RUN_TYPE)
...
print(prediction_shortlist_trn.shape)#, prediction_dist_trn.shape)
...
hc = GraphCollatorV2(head_net, params["num_labels"], 0, num_hard_neg=params["num_HN_shortlist"])
print ('Collator created')
...
for epoch in range(params["num_HN_epochs"]):
    ...
    num_batches = len(head_train_loader.dataset) // params["batch_size"]
#     scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(head_optimizer, num_batches)
    mean_loss = 0
    for batch_idx, batch_data in enumerate(head_train_loader):
        t1 = time.time()
        head_net.zero_grad()
...
#         scheduler.step()
    ...
    epoch_train_end_time = time.time()
    mean_loss /= len(head_train_loader.dataset)
    print("HN Epoch: {}, loss: {}, time: {} sec".format(epoch, mean_loss, epoch_train_end_time - epoch_train_start_time))
    logging.info("HN Epoch: {}, loss: {}, time: {} sec".format(epoch, mean_loss, epoch_train_end_time - epoch_train_start_time))

    if (epoch % 4 == 0): #in params["adjust_lr_epochs"]):
        for param_group in head_optimizer.param_groups:
            param_group['lr'] = param_group['lr'] * params["dlr_factor"]
...
t1 = time.time()
pred = validate(head_net, params, partition_indices, label_remapping,
label_features, valid_tst_point_features, tst_X_Y_val, tst_exact_remove, tst_X_Y_trn, True, 100)
print("Prediction time Per point(ms): ", ((time.time() - t1) / valid_tst_point_features.shape[0]) * 1000)
print("==================================================================")
print("***params=", params)
