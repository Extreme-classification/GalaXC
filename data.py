...
class Graph():
    ...
    def node_features(self, nodes: np.array) -> np.array:
...
class DatasetGraph(torch.utils.data.Dataset):
    def __init__(self, X_Y, hard_negs):
        self.X_Y = X_Y
        self.res_dict = [self.X_Y.indices[self.X_Y.indptr[i]: self.X_Y.indptr[i + 1]]
                         for i in range(len(self.X_Y.indptr) - 1)]
...
class GraphCollator():
    def __init__(self, model, num_labels, num_random=0,
                 train=1, num_hard_neg=10):
        self.model = model
        self.train = train
        ...
        if(self.train):
            ...
            if(not(batch[0][1] is None)):  # prediction
                ...
                context["indices"] = np.array(
                    [b[2] for b in batch], dtype=np.int64)
