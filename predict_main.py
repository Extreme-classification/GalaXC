...
def update_predicted(row_indices, predicted_batch_labels, predicted_labels, remapping, top_k):
    ...
    if(remapping != None):
        ind[:, 1] = [remapping[x] for x in top_indices.cpu().numpy().flatten('C')]
    else:
...
def update_predicted_shortlist(row_indices, predicted_batch_labels, predicted_labels, shortlist, remapping, top_k=10):
    if(len(predicted_batch_labels.shape) == 1):
        predicted_batch_labels = predicted_batch_labels[None, :]
    m = predicted_batch_labels.shape[0]
    
    top_indices = np.argsort(predicted_batch_labels, axis=1)[:, ::-1][:, :top_k]
    ...
    if(remapping != None):
        ind[:, 1] = [remapping[x] for x in np.ravel(shortlist[np.arange(m)[:, None], top_indices])]
    else:
        ind[:, 1] = [x for x in np.ravel(shortlist[np.arange(m)[:, None], top_indices])]
    
    predicted_labels[ind[:, 0], ind[:, 1]] = np.ravel(top_values)

def run_validation(val_predicted_labels, tst_X_Y_val, tst_exact_remove, tst_X_Y_trn, inv_prop):
    data = []
    indptr = [0]
    indices = []
    for i in range(val_predicted_labels.shape[0]):
        ...
        for _ind, _val in zip(_indices1, _vals1):
            if (_ind not in tst_exact_remove[i]) and (_ind not in tst_X_Y_trn.indices[tst_X_Y_trn.indptr[i]: tst_X_Y_trn.indptr[i + 1]]):
    ...
    _pred = csr_matrix((data, indices, indptr), shape=(val_predicted_labels.shape))
...
def encode_nodes(net, context):
    ...
    embed1 = net.first_layer_enc(context["encoder"]["node_feats"]["node_feats"])
...
def validate(head_net, params, partition_indices, label_remapping,
    ...
    for i in range(len(partition_indices)):
        ...
        label_NGS = HNSW(M=100, efC=300, efS=params["num_shortlist"], num_threads=24)
        label_NGS.fit(label_features[partition_indices[i][0]: partition_indices[i][1]])
        print("Done in ", time.time() - t1)
        t1 = time.time()
        
        tst_label_nbrs = np.zeros((tst_point_features.shape[0], params["num_shortlist"]), dtype=np.int64)
        for i in range(0, tst_point_features.shape[0], BATCH_SIZE):
            print(i)
            _tst_label_nbrs, _ = label_NGS.predict(tst_point_features[i: i + BATCH_SIZE], params["num_shortlist"])
    ...
    acc, _ = run_validation(val_predicted_labels.tocsr(), val_data["val_labels"], tst_exact_remove, tst_X_Y_trn, params["inv_prop"])
